...
import directCartService from "../../utils/directCartService";
...
const CartDebugger = () => {
  ...
  const refreshCart = async () => {
    try {
      const cartData = await directCartService.getCart();
      if (cartData.success) {
  ...
  const handleAddToCart = async (alienId) => {
    try {
      setLoading(true);
      await directCartService.addToCart(alienId, 1);
      await refreshCart();
  ...
  const handleRemoveFromCart = async (alienId) => {
    try {
      setLoading(true);
      await directCartService.removeFromCart(alienId);
      await refreshCart();
  ...
  const handleClearCart = async () => {
    try {
      setLoading(true);
      await directCartService.clearCart();
      await refreshCart();
  ...
  return (
    <div className="p-4 bg-gray-800 rounded-lg">
      <h2 className="text-xl font-bold text-white mb-4">Cart Debugger</h2>
      ...
      <div className="mb-6">
        ...
        <p className="text-gray-300">
          Session ID: {directCartService.getSessionId()}
        </p>
      </div>
